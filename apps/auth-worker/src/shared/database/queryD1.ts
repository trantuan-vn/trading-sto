...
export class GenericQuery<T> {
  ...
  async get(): Promise<Array<T & { id: number; created_at: number; updated_at: number }>> {
    let sql: string;
    let params: any[] = [];
    ...
    try {
      ...
      const results: Array<T & { id: number; created_at: number; updated_at: number }> = [];
      ...
      for (const row of rows) {
        ...
        results.push(result as T & { id: number; created_at: number; updated_at: number });
      }
  ...
  async first(): Promise<(T & { id: number; created_at: number; updated_at: number }) | null> {

...
export function createServiceRoutes(bindingName: string) {
  ...
  app.post('/register', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to register service');
  ...
  app.get('/list', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to get services');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.delete('/cancel/:serviceId', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to cancel service');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.get('/usage/:serviceId', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to get service usage');
      return c.json(errorResponse, status);
    }
  });

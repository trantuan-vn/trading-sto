...
import { OrderSchema, OrderItemSchema } from '../order/domain';
  ...
  return {
    async createPaymentUrl(request: CreatePayment, ipAddr: string, identifier: string): Promise<string> {
      ...
      const vnp_Params: Record<string, any> = {
        ...
        'vnp_OrderInfo': request.orderId || `Thanh toan cho ma GD:${identifier}&${payment.id}`,
    ...
    async processReturn(paymentId: string, params: VNPayReturn): Promise<PaymentResult>{
      ...
      if (payment && payment.status === 'pending') {
        ...
        if (order.finalAmount !== parseInt(params.vnp_Amount) / 100) {
        ...
        await userDO.getStorage().transactionSync(() => {
          ...
          if (order.status === 'PENDING' && params.vnp_ResponseCode === '00') {
            const updateOrder = orders.updateSync(payment.orderId, { status: 'COMPLETED' });
            const updateOrderItems = orderItems.where('orderId', '==', updateOrder.id).getSync();
            for (const item of updateOrderItems) {
    ...
    async refundTransaction(identifier: string, request: CreateRefund, ipAddr: string): Promise<RefundResult> {
      const date = new Date();
      ...
      const data = `${vnp_RequestId}|2.1.0|refund|${vnp_TmnCode}|${request.transactionType}|${identifier}&${payment.id}|${order.finalAmount}|${order.finalAmount * 100}|${vnp_TransactionNo}|${vnp_CreateDate}|${identifier}|${vnp_CreateDate}|${ipAddr}|Hoan tien GD ma:${identifier}&${payment.id} voi ly do:${request.reason}`;

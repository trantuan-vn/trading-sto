...
export function createPaymentRoutes(bindingName: string) {
  ...
  app.post('/create_payment_url', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to create payment URL');
  ...
  app.get('/vnpay_return', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to process return');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.get('/vnpay_ipn', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to process IPN');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.post('/querydr', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to query transaction');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.post('/refund', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, e, 'Failed to process refund');
      return c.json(errorResponse, status);
    }
  });

...
export function createTokenRoutes(bindingName: string) {
  ...
  app.post('/create', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, error, 'Failed to create API token');
  ...
  app.get('/list', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, error, 'Failed to get API tokens');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.delete('/revoke/:tokenId', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, error, 'Failed to revoke API token');
      return c.json(errorResponse, status);
    }
  });
  ...
  app.delete('/revoke-all', async (c) => {
    try {
      ...
      const { errorResponse, status } = await handleError(c, error, 'Failed to revoke all API tokens');
      return c.json(errorResponse, status);
    }
  });

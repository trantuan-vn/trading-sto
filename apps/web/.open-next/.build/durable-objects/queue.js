...
// ../../node_modules/.pnpm/@opennextjs+cloudflare@1.6.5_wrangler@4.33.1/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "ZPfJqmZVHYvoQvzU1xC6D");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "ZPfJqmZVHYvoQvzU1xC6D");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "ZPfJqmZVHYvoQvzU1xC6D");

...
// ../../node_modules/.pnpm/@opennextjs+cloudflare@1.6.5_wrangler@4.33.0/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "e46abb4fd27772f717b95610bd4daa9b",
          "x-isr": "1"
        },
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "nu-mNKMQUE490R1Ed9IAn");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "nu-mNKMQUE490R1Ed9IAn");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "nu-mNKMQUE490R1Ed9IAn");
